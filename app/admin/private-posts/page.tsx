...
export default function PrivatePostsPage() {
  ...
  const handleDelete = async (slug: string) => {
    if (!confirm(`确定要删除私密文章 "${slug}" 吗？`)) {
      return;
    }

    try {
      const res = await fetch(`/api/private-posts/${slug}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">私密文章管理</h1>
              ...
              {posts.map((post) => (
                <tr key={post.slug} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right text-sm space-x-3">
                    <Link
                      href={`/admin/private-editor?slug=${encodeURIComponent(post.slug)}`}
